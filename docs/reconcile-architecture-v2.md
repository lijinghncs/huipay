...
| # | 问题 | 选项 | 建议 |
|---|---|---|---|
| Q1 | 重构力度 | A. 完整版（P0–P4，独立 recon 域）；B. 保守版（仅 P0+P1，不搬 payment/scheduler） | A（🔴1 已证明双写是现实缺陷，半收敛会留下两套写入路径） |
| Q2 | P3 时机 | 立即 / 渠道对账下次功能迭代时顺带 | 视渠道对账近期排期定，见 §六 |
| Q3 | 渠道差异 SHORT 单的 `merchant_id` 归属 | 经 `t_order` 关联回填 / 保持 NULL | 回填（管理端按商户过滤差异的完整性），关联不上保持 NULL |
